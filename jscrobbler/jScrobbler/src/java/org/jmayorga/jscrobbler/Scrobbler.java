...
public class Scrobbler
{
  ...
  public static void main( String[] args )
  {
    ...
    for( Iterator iter = artists.iterator(); iter.hasNext(); )
    {
      ...
      {
        MostKnownTracks knownTracks = scrobbler.getMostKnownTracks( element.getName()
            .trim()
            .replace( ' ', '+' )
            .replaceAll( "&", "%26" ) );
        if( null != knownTracks )
        {
          trackCount += knownTracks.getTracks()
              .size();
        }
      }
    }
  ...
  public Collection<Artist> getSimilarArtists( String artist )
  {
    ...
    try
    {
      URL url = new URL( "http://ws.audioscrobbler.com/1.0/artist/" + artist + "/similar.xml" );
      ...
      digester.addBeanPropertySetter( "similarartists/artist/image_small", "image_small" );
      digester.addBeanPropertySetter( "similarartists/artist/image", "image" );
      digester.addBeanPropertySetter( "similarartists/artist/streamable", "streamable" );
  ...
  public MostKnownTracks getMostKnownTracks( String artist )
  {
    ...
    try
    {
      URL url = new URL( "http://ws.audioscrobbler.com/1.0/artist/" + artist + "/toptracks.xml" );
      digester.setValidating( false );
